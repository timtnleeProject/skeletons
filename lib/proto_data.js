...
function _default(Skeletons) {
  ...
  Skeletons.prototype.checkMinMax = function (opt, depth, type, minName, maxName, valid, valueGetter) {
    if (valid) {
      ...
      if (opt[minName] !== undefined) {
        if (typeof opt[minName] !== 'number') return this.warn(depth, "options.".concat(minName, " must be a number"), 99);
        if (value < opt[minName]) return this.warn(depth, "Skeletons.".concat(type, "({ ").concat(minName, " }), ").concat(value, " is lower than ").concat(opt[minName]), 2);
      }

      if (opt[maxName] !== undefined) {
        if (typeof opt[maxName] !== 'number') return this.warn(depth, "options.".concat(maxName, " must be a number"), 99);
        if (value > opt[maxName]) return this.warn(depth, "Skeletons.".concat(type, "({ ").concat(maxName, " }), ").concat(value, " is greater than ").concat(opt[maxName]), 2);
      }
    } else {
      if (opt[minName] !== undefined) {
        return this.warn(depth, "Skeletons.".concat(type, "({ ").concat(minName, " }), value is not a ").concat(type), 2);
      }

      if (opt[maxName] !== undefined) {
        return this.warn(depth, "Skeletons.".concat(type, "({ ").concat(maxName, " }), value is not a ").concat(type), 2);
      }
    }
  ...
  Skeletons.prototype.ArrayFn = function (opt, depth) {
    ...
    var minMaxCheck = this.checkMinMax(opt, depth, 'Array', 'minLength', 'maxLength', Array.isArray(data_deep), function () {
      return data_deep.length;
    });
  ...
  Skeletons.prototype.StringFn = function (opt, depth) {
    ...
    if (status === 200) {
      ...
      var _status = this.checkMinMax(opt, depth, 'String', 'minLength', 'maxLength', typeof data_deep === 'string', function () {
        return data_deep.length;
      });

      if (_status === 200 && opt.match !== undefined) {
  ...
  Skeletons.prototype.NumberFn = function (opt, depth) {
    ...
    var minMaxCheck = this.checkMinMax(opt, depth, 'Number', 'min', 'max', typeof data_deep === 'number', function () {
      return data_deep;
    });
    if (minMaxCheck !== 200) return;
  };
